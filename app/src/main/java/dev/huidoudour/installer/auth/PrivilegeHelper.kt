...
object PrivilegeHelper {
    ...
    fun getActiveDhizukuPackage(context: Context): String? {
        ...
        if (dpm != null) {
            if (dpm.isDeviceOwnerApp(DHIZUKU_PACKAGE) || dpm.isProfileOwnerApp(DHIZUKU_PACKAGE)) {
                Log.d(TAG, "DPM: original is device/profile owner")
                return DHIZUKU_PACKAGE
            }
            if (dpm.isDeviceOwnerApp(DHIZUKU_PACKAGE_CLONE) || dpm.isProfileOwnerApp(DHIZUKU_PACKAGE_CLONE)) {
                Log.d(TAG, "DPM: clone is device/profile owner")
                return DHIZUKU_PACKAGE_CLONE
            }
            Log.d(TAG, "DPM: neither package detected as device/profile owner")
        }
        ...
        Log.d(TAG, "Fallback detection: cloneInstalled=$cloneInstalled, originalInstalled=$originalInstalled")
        return when {
    ...
    fun checkDhizukuStatus(context: Context): PrivilegeStatus {
        ...
        return try {
            ...
            Log.e(TAG, "checkDhizukuStatus error: ${e.message}")
            PrivilegeStatus.NOT_RUNNING
        }
    }
    ...
    fun requestDhizukuPermission(context: Context, onResult: ((Boolean) -> Unit)? = null) {
        ...
        if (dhizukuPackage == null) {
            Log.e(TAG, "No Dhizuku app installed")
        ...
        try {
            if (!Dhizuku.init(context.applicationContext)) {
                // API 连接失败
                Log.w(TAG, "Dhizuku API init failed, package=$dhizukuPackage")
            ...
            Dhizuku.requestPermission(object : DhizukuRequestPermissionListener() {
                override fun onRequestPermission(grantResult: Int) {
                    ...
                    if (granted) {
                        Log.d(TAG, "Dhizuku permission granted")
                    } else {
                        Log.w(TAG, "Dhizuku permission denied")
                    }
                    onResult?.invoke(granted)
                }
            })
        } catch (e: Exception) {
            Log.e(TAG, "Error requesting Dhizuku permission: ${e.message}")
            e.printStackTrace()
            if (dhizukuPackage == DHIZUKU_PACKAGE_CLONE) {
                // 克隆版 API 异常时引导手动授权
                openPrivilegeApp(context, PrivilegeMode.DHIZUKU)
