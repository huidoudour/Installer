...
object TermuxBridge {
     ...
     */
    class PtyInputStream(private val fd: Int) : InputStream() {
     ...
     */
    class PtyOutputStream(private val fd: Int) : OutputStream() {
        ...
        private fun writeByteArray(buffer: ByteArray, offset: Int, length: Int) {
            ...
            if (written < 0) {
                throw IOException("Failed to write to PTY (fd=$fd)")
            }
            if (written < length) {
                // 部分写入，继续重试剩余部分
